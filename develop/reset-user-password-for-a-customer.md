...
Parolayı sıfırlamak, müşterinizin mevcut bir kullanıcı hesabındaki diğer ayrıntıları güncelleştirmeye çok benzer.
...
Belirtilen müşteri kullanıcısının parolasını sıfırlamak için, önce belirtilen müşteri KIMLIĞINI ve hedeflenen kullanıcıyı alın. Ardından, mevcut müşteriyle ilgili bilgileri içeren, ancak yeni bir **Passwordprofile** nesnesi ile yeni bir **customeruser** nesnesi oluşturun. Ardından, **ıaggregatepartner. Customers** koleksiyonunuzu kullanın ve **byıd ()** yöntemini çağırın. Ardından **Users** özelliğini, **byıd ()** yöntemini ve sonra **Patch** yöntemini çağırın.
